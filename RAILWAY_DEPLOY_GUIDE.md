...
2. 같은 리포지토리를 다시 연결
3. **Root Directory**: `server-python` 입력
4. 서비스 이름을 "backend" 또는 "api"로 설정
...
1. 프로젝트 대시보드에서 "New Service" → "GitHub" 선택
2. 같은 리포지토리를 다시 연결 (또는 다른 브랜치 선택)
3. **Root Directory**: 빈칸 (루트 디렉토리)
4. 서비스 이름을 "frontend" 또는 "client"로 설정

**빌드 설정:**
Railway에서 다음 설정을 확인/추가하세요:
...
├── client/              # React 프론트엔드
│   └── src/
├── package.json         # Node.js 설정 및 배포 스크립트
...
1. 백엔드 API 엔드포인트가 정상 작동하는지 확인 (`/docs`에서 Swagger UI 확인)
2. 프론트엔드가 백엔드와 정상 통신하는지 확인
3. 정적 파일들이 올바르게 서빙되는지 확인
