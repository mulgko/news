...
export async function registerRoutes(
  ...
  app.post(api.posts.like.path, async (req, res) => {
    ...
    await storage.incrementLikes(Number(req.params.id));
    res.json({ success: true });
  });

  app.post(api.posts.view.path, async (req, res) => {
    console.log("👁️ 조회수 증가 요청:", req.params.id);
    const post = await storage.getPost(Number(req.params.id));
    if (!post) {
      console.log("❌ 게시물을 찾을 수 없음:", req.params.id);
      return res.status(404).json({ message: "Post not found" });
    }

    try {
      await storage.incrementViews(Number(req.params.id));
      console.log("✅ 조회수 증가 성공:", req.params.id);
      res.json({ success: true });
    } catch (error) {
